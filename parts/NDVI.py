...
def webpage():   #to be called
    ...
    st.title("NDVI Map")
    st.write("""NDVI maps are a powerful tool for monitoring crop health and vegetation vigour. By analysing satellite or aerial imagery, NDVI measures the difference between visible and near-infrared light reflected by plants. This provides an accurate indication of plant vitality, allowing farmers to quickly identify areas of stress, disease, or nutrient deficiency. With this real-time insight, farmers can make timely interventions to improve crop performance, optimize inputs, and ultimately increase yields.

How to use 
1)	Select a rectangular area on the left map.
2)	Coloured map representing NDVI values of each grid of the selected area will be displayed on the right map.
